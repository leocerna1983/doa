	...
	<div class="container">
		<div class="content">
			<nav class="navbar navbar-default" style="margin-top:97px;border-color:#ffffff   !important">
				<div id="navbar" class="navbar-collapse collapse">
					<ul class="nav navbar-nav ">
						<li><a href="list_centrocosto.php">Lista de Centro de Costo</a></li>
						<li  class="active"><a href="add_centrocosto.php">Agregar Centro de Costo</a></li>
					</ul>
				</div>
			</nav>
			<h2>Editar datos de Centro de Costo</h2>
			<hr />

			<?php
			// escaping, additionally removing everything that could be (html/javascript-) code
			$id = mysqli_real_escape_string($con,(strip_tags($_GET["id"],ENT_QUOTES)));
			$consulta = "SELECT nombre, habilitado FROM ccosto WHERE idcc='$id'";
			$sql = mysqli_query($con, $consulta);
			if(mysqli_num_rows($sql) == 0){
				header("Location: list_centrocosto.php");
			}else{
			...
			if(isset($_POST['save'])){
				...
				$sql = "UPDATE ccosto SET nombre ='$nombre', habilitado = '$habilitado' where idcc = ".$id;  
				$update = mysqli_query($con,$sql) or die(mysqli_error());
				if($update){
					//header("Location: edit_centrocosto.php?id=".$id."&pesan=sukses");
					echo "<script language='javascript'>window.location='edit_centrocosto.php?id=".$id."&pesan=sukses'</script>"; 
				}else{
		...
		<div class="modal fade" id="ModalEdit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		  <div class="modal-dialog" role="document">
			...
			<form class="form-horizontal" method="POST" action="edit_centrocosto.php" id="cambioclave" name="cambioclave">
