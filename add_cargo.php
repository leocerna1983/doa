...
<body>
		...
		<div class="content">
			<nav class="navbar navbar-default" style="margin-top:97px;border-color:#ffffff  !important">
				<div id="navbar" class="navbar-collapse collapse">
					<ul class="nav navbar-nav ">
						<li><a href="list_cargo.php">Lista de Cargo</a></li>
						<li  class="active"><a href="add_cargo.php">Agregar Cargo</a></li>
					</ul>
				</div>
			</nav>
			<h2>Agregar datos</h2>
			...
			<?php
				if(isset($_POST['add'])){
					...
					$sql = "INSERT INTO cargo (nombre, habilitado) 
								VALUES('$nombre',$habilitado)";
					$insert = mysqli_query($con, $sql) or die(mysqli_error());
					if($insert){
						echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Cargo creado!</div>';
					}else{
						echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error al grabar el nuevo Cargo!</div>';
					}
					//mysqli_close($con);
				}
			?>
			...
			<form class="form-horizontal" action="" method="post">
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						...
						<a href="list_cargo.php" class="btn btn-sm btn-danger">Cancelar</a>
					</div>
				</div>
			</form>
		</div>
	...
	<div class="modal fade" id="ModalEdit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		  <div class="modal-dialog" role="document">
			...
			<form class="form-horizontal" method="POST" action="add_cargo.php" id="cambioclave" name="cambioclave">
	...
	function CambiarClave(vIdUsuario)
		...
		$("#cambioclave").submit(function(e){
			...
			header("Location: .$url.add_cargo.php");
	    });
