...
<body>
	...
	<div class="content">
			<nav class="navbar navbar-default" style="margin-top:97px;border-color:#ffffff   !important">
				<div id="navbar" class="navbar-collapse collapse">
					<ul class="nav navbar-nav ">
						<li class="active"><a href="list_centrocosto.php">Lista de Centro Costo</a></li>
						<li ><a href="add_centrocosto.php">Agregar Centro Costo</a></li>
					</ul>
				</div>
			</nav>
			...
			<?php
				if (isset($_GET['aksi']) == 'delete') {
					...
					$cek = mysqli_query($con, "SELECT * FROM ccosto WHERE idcc=".$id);
					if(mysqli_num_rows($cek) == 0){
						echo '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
					}else{
						$sqlstr = "DELETE FROM ccosto WHERE idcc = ".$id;
						if ($con->query($sqlstr) === TRUE) {
							echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Centro de Costo eliminado correctamente.</div>';
						} else {
							echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Error, no se pudo borrar el Centro de Costo.</div>';	
						}
					}
				}
			?>
			...
			<table class="table table-striped table-hover">
				...
				<?php
					...
					if($filter!=7){
						$consulta = "SELECT idcc, nombre, habilitado FROM ccosto
					...
					if(mysqli_num_rows($sql) == 0){
						echo '<tr><td colspan="8">No existen Centros de Costo.</td></tr>';
					}else{
						while($row = mysqli_fetch_assoc($sql)){
							...
							echo '
								...
								<td>
									<a href="edit_centrocosto.php?aksi=save&id='.$row['idcc'].'" title="Editar Centro Costo" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
									<a href="list_centrocosto.php?aksi=delete&id='.$row['idcc'].'" title="Eliminar" onclick="return confirm(\'Está seguro de borrar el Centro de Costo: '.$row['nombre'].'?\')" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>
								</td>
							</tr>
							';
						}
	...
	<div class="modal fade" id="ModalEdit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		  <div class="modal-dialog" role="document">
			...
			<form class="form-horizontal" method="POST" action="list_centrocosto.php" id="cambioclave" name="cambioclave">
