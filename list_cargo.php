...
<body>
	...
	<div class="content">
			<nav class="navbar navbar-default" style="margin-top:97px;border-color:#ffffff  !important">
				<div id="navbar" class="navbar-collapse collapse">
					<ul class="nav navbar-nav ">
						<li class="active"><a href="list_cargo.php">Lista de Cargo</a></li>
			...
			<?php
				if (isset($_GET['aksi']) == 'delete') {
					...
					$cek = mysqli_query($con, "SELECT idcargo, nombre, habilitado FROM cargo WHERE idcargo=".$id);
					if(mysqli_num_rows($cek) == 0){
						echo '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
					}else{
						$sqlstr = "DELETE FROM cargo WHERE idcargo = ".$id;
						if ($con->query($sqlstr) === TRUE) {
							echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Cargo eliminado correctamente.</div>';
						} else {
							echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Error, no se pudo borrar el Cargo.</div>';	
						}
					}
				}
			?>
			...
			<table class="table table-striped table-hover">
				...
				<?php
					if($filter!=7){
						$consulta = "SELECT idcargo, nombre, habilitado FROM cargo
						WHERE habilitado=$filter ORDER BY idcargo ASC ";
					}else{
						$consulta = "SELECT idcargo, nombre, habilitado FROM cargo
					...
					if(mysqli_num_rows($sql1) == 0){
						echo '<tr><td colspan="8">No existen Cargos.</td></tr>';
					}else{
						while($row = mysqli_fetch_assoc($sql1)){
							...
							echo '</td>
								<td>
									<a href="edit_cargo.php?aksi=save&id='.$row['idcargo'].'" title="Editar Cargo" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
									<a href="list_cargo.php?aksi=delete&id='.$row['idcargo'].'" title="Eliminar" onclick="return confirm(\'Está seguro de borrar el Cargo: '.$row['nombre'].'?\')" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>
								</td>
							</tr>
							';
						}
	...
	<div class="modal fade" id="ModalEdit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		  <div class="modal-dialog" role="document">
			...
			<form class="form-horizontal" method="POST" action="list_cargo.php" id="cambioclave" name="cambioclave">
	...
	<script type="text/javascript">
		...
		$("#cambioclave").submit(function(e){
			...
			header("Location: .$url.list_cargo.php");
	    });
	</script>
</body>
</html>
